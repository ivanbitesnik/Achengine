...
namespace Achengine
{
    namespace
    {
        ...
        std::string ResolveTexturePath(const std::string& inputPath, const std::string& modelDirectory)
        {
            if (inputPath.empty())
            {
                return "";
            ...
            {
                candidates.push_back(NormalizePath(modelDirectory + "/" + normalized));

                const std::string fileName = GetFileName(normalized);
                candidates.push_back(NormalizePath(modelDirectory + "/" + fileName));
                candidates.push_back(NormalizePath(modelDirectory + "/textures/" + fileName));
                candidates.push_back(NormalizePath(modelDirectory + "/Textures/" + fileName));
            }
    ...
    UModelMesh::UModelMesh(const std::string& modelPath, const std::string& shaderPath)
    {
    ...
    UModelMesh::~UModelMesh()
    {
        CleanupSubmeshTextures();
    }

    bool UModelMesh::ReloadModel(const std::string& modelPath)
    {
        if (!LoadFromFile(modelPath))
        {
            return false;
    ...
    void UModelMesh::GenerateVertexArray()
    {
    ...
    void UModelMesh::CleanupSubmeshTextures()
    {
        for (FImportedSubMesh& subMesh : m_SubMeshes)
        {
    ...
    Texture* UModelMesh::TryLoadTexturePath(const std::string& texturePath, bool& outWasLoaded)
    {
        outWasLoaded = false;
        const std::string resolvedPath = ResolveTexturePath(texturePath, m_ModelDirectory);
        ...
        outWasLoaded = true;
        return Texture2D::Create(resolvedPath);
    }

    bool UModelMesh::LoadFromFile(const std::string& modelPath)
    {
#ifdef ACHENGINE_ENABLE_ASSIMP
